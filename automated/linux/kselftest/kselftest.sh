...
while getopts "t:s:u:p:L:S:b:g:e:c:h" opt; do
    case "${opt}" in
        t) TESTPROG="${OPTARG}" ;;
        ...
        e)
            ...
            ;;
        c)
            export KSELFTEST_SUBSET="${OPTARG}"
            ;;
        h|*) usage ;;
    esac
done
...
if [ -z "${KSELFTEST_SUBSET}" ]; then
    ./run_kselftest.sh 2>&1 | tee "${LOGFILE}"
else
    ./run_kselftest.sh -c ${KSELFTEST_SUBSET} 2>&1 | tee "${LOGFILE}"
fi
parse_output
